...
void aes_encoder::init(const fc::sha256 &key, const fc::uint128 &init_value) {
   ...
   *    is 128 bits */
   if( 1 !=
       EVP_EncryptInit_ex(my->ctx, EVP_aes_256_cbc(), NULL, (unsigned char *) &key, (unsigned char *) &init_value)) {
...
uint32_t aes_encoder::encode(const char *plaintxt, uint32_t plaintext_len, char *ciphertxt) {
   ...
   *       */
   if( 1 != EVP_EncryptUpdate(my->ctx, (unsigned char *) ciphertxt, &ciphertext_len, (const unsigned char *) plaintxt,
                              plaintext_len)) {
...
void aes_decoder::init(const fc::sha256 &key, const fc::uint128 &init_value) {
   ...
   *    is 128 bits */
   if( 1 !=
       EVP_DecryptInit_ex(my->ctx, EVP_aes_256_cbc(), NULL, (unsigned char *) &key, (unsigned char *) &init_value)) {
      FC_THROW_EXCEPTION(aes_exception, "error during aes 256 cbc encryption init",
                         ("s", ERR_error_string(ERR_get_error(), nullptr)));
   }
   EVP_CIPHER_CTX_set_padding(my->ctx, 0);
...
uint32_t aes_decoder::decode(const char *ciphertxt, uint32_t ciphertxt_len, char *plaintext) {
   ...
   *       */
   if( 1 != EVP_DecryptUpdate(my->ctx, (unsigned char *) plaintext, &plaintext_len, (const unsigned char *) ciphertxt,
                              ciphertxt_len)) {
...
openssl_thread_config::~openssl_thread_config()
{
  if (CRYPTO_THREADID_get_callback() != NULL &&
      CRYPTO_THREADID_get_callback() == &get_thread_id)
  {
