...
inline const std::string& base64m_chars()
{
...
std::string base64m_encode( const std::string& enc ) {
  ...
  return base64m_encode( (unsigned char const*)s, enc.size() );
}
std::string base64m_encode(unsigned char const* bytes_to_encode, unsigned int in_len) {
  ...
  while (in_len--) {
    ...
    if (i == 3) {
      ...
      for(i = 0; (i <4) ; i++)
        ret += base64m_chars()[char_array_4[i]];
      i = 0;
    }
  }
  ...
  if (i)
  {
    ...
    for (j = 0; (j < i + 1); j++)
      ret += base64m_chars()[char_array_4[j]];
...
std::string base64m_decode(std::string const& encoded_string) {
  ...
  while (in_len-- && ( encoded_string[in_] != '=') && is_base64m(encoded_string[in_])) {
    char_array_4[i++] = encoded_string[in_]; in_++;
    if (i ==4) {
      for (i = 0; i <4; i++)
        char_array_4[i] = base64m_chars().find(char_array_4[i]);
  ...
  if (i) {
    ...
    for (j = 0; j <4; j++)
      char_array_4[j] = base64m_chars().find(char_array_4[j]);
