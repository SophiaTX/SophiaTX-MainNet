...
{
   public:
      wallet_api( const wallet_data& initial_data, const sophiatx::protocol::chain_id_type& _sophiatx_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
...
FC_REFLECT( sophiatx::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( sophiatx::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( sophiatx::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( sophiatx::wallet::authority_type, (owner)(active) )

FC_API( sophiatx::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
