...
const sophiatx::chain::database& chain_plugin::db() const { return my->db; }
...
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( SOPHIATX_CHAIN_ID_NAME ), "chain ID to connect to")
#endif
         ;
}
...
bool chain_plugin::accept_block( const sophiatx::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const sophiatx::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
bool chain_plugin::block_is_on_preferred_chain(const sophiatx::chain::block_id_type& block_id )
{
...
void chain_plugin::check_time_in_block( const sophiatx::chain::signed_block& block )
{
...
sophiatx::chain::signed_block chain_plugin::generate_block( const fc::time_point_sec when, const account_name_type& witness_owner,
