...
public:
   ...
   /// smt_setup_emissions
   time_point_sec       schedule_time = SOPHIATX_GENESIS_TIME;
   sophiatx::protocol::
   smt_emissions_unit   emissions_unit;
   uint32_t             interval_seconds = 0;
   uint32_t             interval_count = 0;
   time_point_sec       lep_time = SOPHIATX_GENESIS_TIME;
   time_point_sec       rep_time = SOPHIATX_GENESIS_TIME;
   asset                lep_abs_amount = asset( 0, SOPHIATX_SYMBOL );
...
FC_REFLECT_ENUM( sophiatx::chain::smt_token_object::smt_phase,
                 (account_elevated)
                 (setup_completed)
)

FC_REFLECT( sophiatx::chain::smt_token_object::smt_market_maker_state,
   (sophiatx_balance)
   (token_balance)
   (reserve_ratio)
)
