...
/**
 * Class responsible for holding non SOPHIATX balance (SMT one) for given account.
...
FC_REFLECT( sophiatx::chain::account_regular_balance_object,
   (id)
   (owner)
   (balance)
)
...
CHAINBASE_SET_INDEX_TYPE( sophiatx::chain::account_regular_balance_object, sophiatx::chain::account_regular_balance_index )
CHAINBASE_SET_INDEX_TYPE( sophiatx::chain::account_rewards_balance_object, sophiatx::chain::account_rewards_balance_index )
