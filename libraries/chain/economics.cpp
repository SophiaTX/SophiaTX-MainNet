...
void economic_model_object::init_economics(share_type _init_supply, share_type _total_supply){
   ...
   mining_pool_from_coinbase = coinbase * protocol::sophiatx_config::get<int64_t>("SOPHIATX_MINING_POOL_PERCENTAGE") / SOPHIATX_100_PERCENT;
   interest_pool_from_coinbase = coinbase * protocol::sophiatx_config::get<int64_t>("SOPHIATX_INTEREST_POOL_PERCENTAGE") / SOPHIATX_100_PERCENT;
   promotion_pool = coinbase * protocol::sophiatx_config::get<int64_t>("SOPHIATX_PROMOTION_POOL_PERCENTAGE") / SOPHIATX_100_PERCENT;
   initial_promotion_pool = promotion_pool;
   init_supply = _init_supply;
   total_supply = _total_supply;
   coinbase_block_reward = mining_pool_from_coinbase / protocol::sophiatx_config::get<int64_t>("SOPHIATX_COINBASE_BLOCKS");
}

share_type economic_model_object::get_mining_reward(uint32_t block_number)const{
   uint32_t blocks_to_coinbase_end = protocol::sophiatx_config::get<uint32_t>("SOPHIATX_COINBASE_BLOCKS") - block_number;
   ...
   reward += mining_pool_from_fees / (protocol::sophiatx_config::get<uint32_t>("SOPHIATX_BLOCKS_PER_DAY") * 7);
   return reward;
}
...
share_type economic_model_object::withdraw_mining_reward(uint32_t block_number, uint32_t nominator, uint32_t denominator){
   //mining reward consist of coinbase reward, which uniformly distributes mining pool among SOPHIATX_COINBASE_BLOCKS rewards,
   // and fees rewards, where each block is rewarded one 1/(SOPHIATX_BLOCKS_PER_DAY * 7) of the current pool.
   if( block_number <= protocol::sophiatx_config::get<uint32_t>("SOPHIATX_INTEREST_DELAY") )
      return 0;
   ...
   share_type reward_from_fees = mining_pool_from_fees / (protocol::sophiatx_config::get<uint32_t>("SOPHIATX_BLOCKS_PER_DAY") * 7);
...
#define SOPHIATX_TOTAL_INTERESTS ((protocol::sophiatx_config::get<uint64_t>("SOPHIATX_TOTAL_SUPPLY") \
        - protocol::sophiatx_config::get<uint64_t>("SOPHIATX_INIT_SUPPLY")) * \
protocol::sophiatx_config::get<uint64_t>("SOPHIATX_INTEREST_POOL_PERCENTAGE") / uint64_t(SOPHIATX_100_PERCENT))
...
share_type economic_model_object::withdraw_interests(share_type holding, uint32_t period) {
   try {
      ...
      share_type total_coinbase_for_period = share_type(std::min(uint64_t(interest_pool_from_coinbase.value),
                                                                 ...
                                                                 protocol::sophiatx_config::get<uint64_t>("SOPHIATX_COINBASE_BLOCKS"))));
      ...
      share_type fees_reward = (
            uint128_t(interest_pool_from_fees.value * SOPHIATX_INTEREST_BLOCKS / protocol::sophiatx_config::get<uint64_t>("SOPHIATX_INTEREST_FEES_TIME")) *
...
void economic_model_object::add_fee(share_type fee) {
   auto to_burn = fee * protocol::sophiatx_config::get<int64_t>("SOPHIATX_BURN_FEE_PERCENTAGE") / SOPHIATX_100_PERCENT;
   fee = fee - to_burn;
   auto to_mining_pool = fee * protocol::sophiatx_config::get<int64_t>("SOPHIATX_MINING_POOL_PERCENTAGE") / SOPHIATX_100_PERCENT;
   auto to_promotion_pool = fee * protocol::sophiatx_config::get<int64_t>("SOPHIATX_PROMOTION_POOL_PERCENTAGE") / SOPHIATX_100_PERCENT;
...
share_type economic_model_object::get_available_promotion_pool(uint32_t block_number) const{
   uint128_t blocks_to_coinbase_end = protocol::sophiatx_config::get<uint128_t>("SOPHIATX_COINBASE_BLOCKS") - block_number;
   //share_type locked_pool = promotion_pool_per_day * blocks_to_coinbase_end / SOPHIATX_BLOCKS_PER_DAY;
   share_type locked_pool = (((((uint128_t)initial_promotion_pool.value * (uint128_t)262144) / protocol::sophiatx_config::get<uint128_t>("SOPHIATX_COINBASE_BLOCKS")) * blocks_to_coinbase_end ) / (uint128_t)262144).to_uint64();
