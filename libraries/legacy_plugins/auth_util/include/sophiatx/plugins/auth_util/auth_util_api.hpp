...
{
   public:
      auth_util_api( const sophiatx::app::api_context& ctx );
...
FC_REFLECT( sophiatx::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( sophiatx::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( sophiatx::plugin::auth_util::auth_util_api,
