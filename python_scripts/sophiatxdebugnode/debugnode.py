...
from sophiatxapi.sophiatxnoderpc import SophiaTXNodeRPC

class DebugNode( object ):
   """ Wraps the sophiatxd debug node plugin for easier automated testing of the SophiaTX Network"""

   def __init__( self, sophiatxd, data_dir, args='', plugins=[], apis=[], sophiatxd_out=None, sophiatxd_err=None ):
      """ Creates a sophiatxd debug node.
      ...
      For all other requests, the python-sophiatx library should be used.

      args:
         sophiatxd -- The string path to the location of the sophiatxd binary
         data_dir -- The string path to an existing sophiatxd data directory which will be used to pull blocks from.
         args -- Other string args to pass to sophiatxd.
         ...
         sophiatxd_stdout -- A stream for sophiatxd's stdout. Default is to pipe to /dev/null
         sophiatxd_stderr -- A stream for sophiatxd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._sophiatxd_bin = None
      self._sophiatxd_lock = None
      self._sophiatxd_process = None
      self._temp_data_dir = None

      self._sophiatxd_bin = Path( sophiatxd )
      if( not self._sophiatxd_bin.exists() ):
         raise ValueError( 'sophiatxd does not exist' )
      if( not self._sophiatxd_bin.is_file() ):
         raise ValueError( 'sophiatxd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed sophiatx data directory' )
      ...
      if( sophiatxd_out != None ):
         self.sophiatxd_out = sophiatxd_out
      else:
         self.sophiatxd_out = self._FNULL

      if( sophiatxd_err != None ):
         self.sophiatxd_err = sophiatxd_err
      else:
         self.sophiatxd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._sophiatxd_lock = Lock()


   def __enter__( self ):
      self._sophiatxd_lock.acquire()
      ...
      sophiatxd = [ str( self._sophiatxd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      sophiatxd.extend( self._args )

      self._sophiatxd_process = Popen( sophiatxd, stdout=self.sophiatxd_out, stderr=self.sophiatxd_err )
      self._sophiatxd_process.poll()
      sleep( 5 )
      if( not self._sophiatxd_process.returncode ):
         self._rpc = SophiaTXNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "sophiatxd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._sophiatxd_process != None ):
         self._sophiatxd_process.poll()

         if( not self._sophiatxd_process.returncode ):
            self._sophiatxd_process.send_signal( SIGINT )

            sleep( 7 )
            self._sophiatxd_process.poll()

            if( not self._sophiatxd_process.returncode ):
               self._sophiatxd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._sophiatxd_process.poll()

               if( self._sophiatxd_process.returncode ):
                  loggin.error( 'sophiatxd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._sophiatxd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._sophiatxd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key sophiatx debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is SOPHIATX_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both sophiatxd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--sophiatxd', '-s', type=str, required=True, help='The location of a sophiatxd binary to run the debug node' )
      ...
      sophiatxd = Path( args.sophiatxd )
      if( not sophiatxd.exists() ):
         print( 'Error: sophiatxd does not exist.' )
         return

      sophiatxd = sophiatxd.resolve()
      if( not sophiatxd.is_file() ):
         print( 'Error: sophiatxd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed sophiatxd data directory' )
      ...
      debug_node = DebugNode( str( sophiatxd ), str( data_dir ), sophiatxd_err=sys.stderr )
