...
{{
   public:
      {plugin_name}_api( const sophiatx::app::api_context& ctx );
...
class {plugin_name}_plugin : public sophiatx::app::plugin
{{
   public:
      {plugin_name}_plugin( sophiatx::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <sophiatx/app/api_context.hpp>
#include <sophiatx/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( sophiatx::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      sophiatx::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( sophiatx::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const sophiatx::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( sophiatx::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( sophiatx::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( sophiatx::app::application* app )
   : plugin(app)
{{
...
SOPHIATX_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
