...
SOPHIATXD_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! sophiatxdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/sophiatxdpid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/sophiatxd-full/bin/sophiatxd /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! sophiatxdsync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/sophiatxd`
  kill -9 $RUN_SV_PID
fi
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    SOPHIATXD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $SOPHIATXD_PID
    echo sophiatxdsync: waiting for sophiatxd to exit cleanly
    while [ -e /proc/$SOPHIATXD_PID ]; do sleep 0.1; done
    echo sophiatxdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo sophiatxdsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! sophiatxdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo sophiatxdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! sophiatxdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo sophiatxdsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! sophiatxdsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo sophiatxdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifysophiatxdsync: sophiatxdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/sophiatxd`
    kill -9 $RUN_SV_PID
  fi
