          ...
          dir('bin') {
              ...
              script {
                  ...
                  if( params.build_as_testnet ) {
                     ...
                     sh 'tar -czf ${ARCHIVE_NAME} alexandria_deamon cli_wallet sophiatxd sophiatx_light testnet_config.ini' //create tar file
                  } else {
                     sh 'cp ${WORKSPACE}/contrib/fullnode_config.ini .'//copy configs
                     sh 'cp ${WORKSPACE}/contrib/witness_config.ini .'//copy configs
                     sh 'tar -czf ${ARCHIVE_NAME} alexandria_deamon cli_wallet sophiatxd sophiatx_light fullnode_config.ini witness_config.ini' //create tar file
                  }
              }
              archiveArtifacts '*.gz'
          }
