      ...
      if( !params.build_as_debug ) {
        try {
          ...
          if( params.build_as_testnet ) {
           ...
           sh "tar -czf ${ARCHIVE_NAME} cli_wallet sophiatxd testnet_config.ini" //create tar file
           } else {
           sh "cp ${WORKSPACE}/contrib/fullnode_config.ini ."//copy configs
           sh "cp ${WORKSPACE}/contrib/witness_config.ini ."//copy configs
           sh "tar -czf ${ARCHIVE_NAME} cli_wallet sophiatxd fullnode_config.ini witness_config.ini" //create tar file
         }
       }
       archiveArtifacts '*.gz'
     }
